...
item = offer()
canonical, html_text, label, record = mod.make_article(item)
assert canonical and label == "Promo code"
assert record["merchant"] == "Zara" and record["category"] == "Fashion"
...
assert "/brand/zara/" in html_text
assert "/fashion/" in html_text
...
assert label2 == "Direct deal" and record2["label"] == "Direct deal"
...
print("SEO GENERATOR PREFLIGHT PASS: canonical published contract, code/direct, noise sanitization, CTA destination, internal navigation, and canonical uniqueness")

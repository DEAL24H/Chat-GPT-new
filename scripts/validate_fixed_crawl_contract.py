...
def main():
 ...
 assert CATS==list(EXPECTED_CATEGORIES), CATS
 ...
 assert len(set(s['merchant'] for s in sources))==120
 ...
 for merchant,market,url in targets:
  assert url.startswith(('http://','https://')), (merchant,market,url)
  assert url.strip(), (merchant,market,url)

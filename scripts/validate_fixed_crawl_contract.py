...
def main():
 ...
 source_brands={s['merchant'].strip().casefold() for s in sources};assert len(source_brands)==120
 roots=[];locales=[];targets=[]
 for source in sources:
  root=source['official_homepage'];roots.append(root);targets.append((source['merchant'],'Gốc / Quốc tế',root))
  for row in locales_for(source['merchant']):locales.append(row['url']);targets.append((source['merchant'],row['market'],row['url']))

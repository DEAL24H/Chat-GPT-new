...
TIMEOUT = 15
UA = "Mozilla/5.0 (compatible; Deal24HOfferLinkValidator/3.0; +https://deal24h.net/)"
...
def live_destination(item):
    ...
    if expected_pct is not None:
        if destination_pct is None:
            return False, f"DESTINATION_MISSING_EXPECTED_DISCOUNT_{expected_pct}", final
...
def main():
    ...
    if errors:
        print(f"OFFER LINK VALIDATION FAILED: errors={len(errors)} live_verified={verified} total={len(data)}")
        for error in errors[:100]:
            print(error)
        raise SystemExit(1)
    print(f"OFFER LINK VALIDATION OK: offers={len(data)} live_verified={verified}; every published destination is live, official-domain, purchase-capable and offer-content consistent")

...
ROOT=Path(__file__).resolve().parents[1]; DATA=ROOT/'data/news.json'; CATALOG=ROOT/'data/brand_catalog.json'
TIMEOUT=15; UA='Mozilla/5.0 (compatible; Deal24HOfferLinkValidator/6.0; +https://deal24h.net/)'
...
def purchase(v):
    ...
    p=urlparse(v); s=f'{p.path} {p.query}'
    return not (BAD.search(s) and not SHOP.search(s)) and bool(SHOP.search(s) or (p.hostname or '').lower().startswith(('shop.','store.')))
def live(item):
    ...
    if not (SHOP.search(f'{urlparse(f).path} {urlparse(f).query}') or CTA.search(text) or re.search(r'addtocart|add-to-cart|buy-now|checkout',html) or re.search(r'\$|€|£|¥|\b(?:USD|EUR|GBP|CAD|AUD)\b',text)):
        return 'failed','NO_COMMERCE_SIGNAL',f
    return 'live_verified','LIVE_BRAND_SALES_DESTINATION_VERIFIED',f

def main():
    data=json.loads(DATA.read_text()); catalog=json.loads(CATALOG.read_text()).get('categories',{})
    domains={str(e.get('name','')).casefold():str(e.get('domain','')) for es in catalog.values() for e in es}
    ...
    for item in data:
        ...
        item['purchase_url_verification_status']='live_verified'; item['purchase_url_verification_reason']=vr; item['purchase_url_verified_at']=now
        item['final_purchase_url']=item['promotion_url']=item['url']=final; item['published_offer_authority']='assistant_verified_source_plus_live_brand_sales_destination'; published.append(item)
    ...
    print('OFFER LINK VALIDATION PASS: each published offer has a live purchase/sales destination on the verified brand domain')

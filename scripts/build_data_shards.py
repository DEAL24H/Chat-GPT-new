...
def compact(item, shard):
    return {
        ...
        "promotion_url": item.get("promotion_url") or item.get("url") or item.get("source_url", ""),
...
def main():
    ...
    # Remove every previously generated shard first. Only the six priority
    # categories below are allowed back into the active browser data layer.
    ...
    manifest = {"version": 1, "categories": list(CATEGORY_SLUGS), "shards": {}, "search": "search-index.json"}
    ...
    print(f"Built {len(shards)} active data shards and {len(search)} search records{ignored}")

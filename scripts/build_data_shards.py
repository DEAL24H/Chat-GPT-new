...
def main():
 ...
 print(f"Built {sum(bool(v) for v in shards.values())} active data shards, {len(shards)} total category shards and {len(search)} search records"+(f"; skipped non-priority categories: {', '.join(sorted(skipped))}" if skipped else ''))

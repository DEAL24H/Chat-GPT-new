...
def active(item):return isinstance(item,dict) and item.get('status') not in {'expired','inactive'} and item.get('offer_qualified') is True and item.get('purchase_url_verification_status')=='live_verified' and bool(item.get('final_purchase_url'))
...
def main():
 ...
 search.sort(key=lambda x:(norm(x['merchant']),x['id'])); search_payload={'version':1,'count':len(search),'items':search}

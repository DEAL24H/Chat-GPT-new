...
raw=json.loads(DATA.read_text(encoding='utf-8')); items=raw if isinstance(raw,list) else raw.get('items',[]); rows=[]; seen_publications=set()
for x in items:
 ...
 if x.get('purchase_url_verification_status') not in {'live_verified','official_destination_pending'}: raise SystemExit(f'Supabase contract: purchase URL is not official for {merchant}: {x.get("purchase_url_verification_status")}')

...
def main():
    ...
    for index, item in enumerate(data):
        category = str(item.get("category") or "").strip(); title = str(item.get("title") or "").strip(); content = str(item.get("content") or "").strip()
        destination = str(item.get("final_purchase_url") or "").strip(); promotion_url = str(item.get("promotion_url") or "").strip(); source = str(item.get("source_url") or "").strip(); code = str(item.get("code") or "").strip()
        ...
        if not same_domain(destination, item.get("official_homepage") or source): failures.append((index, "destination_left_official_domain", destination))

...
FORBIDDEN_ROOTS = {"w3.org", "bing.com", "yahoo.com", "facebook.com", "instagram.com", "tiktok.com", "x.com", "twitter.com", "wikipedia.org"}
...
def forbidden(domain):
    return domain in FORBIDDEN_ROOTS or any(domain.endswith("." + root) for root in FORBIDDEN_ROOTS) or domain in {"google.com", "googleusercontent.com"}
...
def main():
    ...
    for category, expected_count in EXPECTED.items():
        ...
        for entry in entries:
            ...
            name = str(entry.get("name", "")).strip(); key = name.casefold(); domain = normalize_domain(entry.get("domain"))
    ...
    print("CATALOG LOCK VALID: 6 categories x 89 = 534 brands; all 534 have non-placeholder official domains")

...
def main():
    ...
    if not isinstance(categories, dict) or set(categories) != set(EXPECTED):
        raise SystemExit("CATALOG VALIDATION FAILED: expected exactly 6 priority categories")
    names = set(); seo_only = 0; verified = 0
    for category, expected_count in EXPECTED.items():
        ...
        for entry in entries:
            ...
            name = str(entry.get("name", "")).strip(); key = name.casefold(); domain = str(entry.get("domain", "")).strip()
            ...
            if domain:
                verified += 1
            elif entry.get("seo_only") is True:
                seo_only += 1
            else:
                raise SystemExit(f"CATALOG VALIDATION FAILED: brand needs verified domain or seo_only=true: {name}")
            names.add(key)
    if len(names) != TARGET:
        raise SystemExit(f"CATALOG VALIDATION FAILED: expected {TARGET} brands, got {len(names)}")
    print(f"CATALOG VALID: 6 categories x 89 = 534 brands; verified-domain={verified}, SEO-only-pending-domain={seo_only}")

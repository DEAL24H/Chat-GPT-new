...
async function load(){
  const params=new URLSearchParams(location.search); const id=params.get('id'); const url=params.get('url');
  try{
    const r=await fetch('data/news.json',{cache:'no-store'}); if(!r.ok) throw Error(r.status);
    const d=await r.json(); const item=(d.items||[]).find(x=>(id&&x.id===id)||(url&&x.url===url));
    ...
    $('#post').innerHTML=`
      ...
      <div class="post-body"><p>${esc(item.summary||'Nội dung đang được cập nhật.')}</p></div>

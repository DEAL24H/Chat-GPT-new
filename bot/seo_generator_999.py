...
def brand_seo_summary(brand,category,items):
    """Permanent SEO exists independently; offer facts are added only when verified data exists."""
    lines=[f"{brand} coupons and deals",f"{brand} {category.lower()} products and promotions"]
    if items:
        for x in items[:2]:
            code=str(x.get("code") or "").strip(); discount=str(x.get("discount") or "").strip(); title=re.sub(r"\s+"," ",str(x.get("title") or "")).strip(); pct=re.search(r"\b(\d{1,3})\s*%\s*(?:off|discount)\b",f"{discount} {title}",re.I)
            if pct: lines.append(f"{brand} currently has {pct.group(1)}% off")
            elif code: lines.append(f"{brand} promotion code {code}")
            elif title: lines.append(f"{brand} deal: {title[:90]}")
    return '<section class="brand-seo" aria-labelledby="brand-seo-title"><h2 id="brand-seo-title">'+esc(brand)+' coupons, deals and promotions</h2><ul>'+''.join(f'<li>{esc(x)}.</li>' for x in lines[:4])+'</ul></section>'
...
def main():
    ...
    for cat,entries in CATALOG.items():
        cs=CATEGORY_SLUGS.get(cat,brand_slug(cat)); cu=f"{BASE}/{cs}/"; cat_urls.append(cu); active=bc.get(cat,[]); cards="".join(card(x) for x in active[:60]) or '<p>No active coupons or deals are currently listed.</p>'; links="".join(f'<li><a href="/brand/{brand_slug(e["name"])}/">{esc(e["name"])} coupons & deals</a></li>' for e in entries); body=f'<section class="hero"><p class="eyebrow">COUPON CODES · PROMO CODES · DEALS</p><h1>{esc(cat)} Coupons, Promo Codes & Deals</h1><p class="lead">Active coupon codes, promo codes and official deals from merchant sources.</p></section><section><h2>Latest {esc(cat)} offers</h2><div class="grid">{cards}</div></section><section><h2>Brands</h2><ul>{links}</ul></section>'; write(ROOT/cs/"index.html",page(f"{cat} Coupons, Promo Codes & Deals | DEAL 24H",f"Find active {cat.lower()} coupon codes, promo codes and deals from official merchant sources.",cu,body,s={"@context":"https://schema.org","@type":"CollectionPage","name":f"{cat} Coupons, Promo Codes & Deals","url":cu}))
        for e in entries:
            b=e["name"]; u=f"{BASE}/brand/{brand_slug(b)}/"; items=bb.get(b.lower(),[]); p=ROOT/"brand"/brand_slug(b)/"index.html"; img=logo(b); ih=f'<img class="brandhero-img" src="{esc(img)}" alt="{esc(b)} logo" loading="eager">' if img else '<span class="brandfallback" aria-hidden="true">B</span>'; permanent=brand_intro(b,cat); seo=brand_seo_summary(b,cat,items); offer_section=f'<section><h2>All active {esc(b)} offers</h2><div class="grid">{"".join(card(x) for x in items)}</div></section>' if items else '<section><h2>Current offers</h2><p>No active coupon code or deal is currently listed for this brand. The page remains available and will update when a verified official offer is found.</p></section>'; body=f'<section class="hero"><div class="brandhero"><div class="brandhero-logo">{ih}</div><div><p class="eyebrow">{esc(cat.upper())} · COUPONS & DEALS</p><h1>{esc(b)} Coupons, Promo Codes & Deals</h1></div></div><p class="lead">Find current {esc(b)} promotions and verified official offers on DEAL24H.</p></section>{permanent}{seo}{offer_section}<p><a href="/{cs}/">← More {esc(cat)} offers</a></p>'; els=[{"@type":"ListItem","position":i+1,"name":f"{b} "+("coupon code "+str(x.get("code")) if x.get("code") else "deal"),"url":x.get("final_purchase_url")} for i,x in enumerate(items)]; schema={"@context":"https://schema.org","@graph":[{"@type":"Organization","name":b,"url":official_homepage(b)},{"@type":"WebPage","name":f"{b} Coupons, Promo Codes & Deals","url":u}]};
            if els: schema["@graph"].append({"@type":"ItemList","name":f"Active {b} offers","numberOfItems":len(items),"itemListElement":els})
            write(p,page(f"{b} Coupons, Promo Codes & Deals | DEAL 24H",f"Find current {b} coupon codes, promo codes and official deals from official merchant sources on DEAL24H.",u,body,s=schema)); brand_urls.append(u)

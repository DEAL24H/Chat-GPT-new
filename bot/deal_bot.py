...
H={'User-Agent':'Deal24H/6.0 (+https://deal24h.net/ official promotion crawler)','Accept':'text/html,application/xhtml+xml','Accept-Language':'en-US,en;q=0.9'}
...
def extract(response,source):
 ...
 out=[];seen=set()
 for b in blocks:
  text=clean(b.get_text(' ',strip=True));cs=codes(text)
  if not 30<=len(text)<=1800 or NOISE.search(text) or not PROMO.search(text) or not(BENEFIT.search(text) or cs):continue
  title=title_for(b,text)
...
def load_sources():
 data=json.loads(SELECTION.read_text());rows=[]
 for cat in CATS:
  for x in data.get('categories',{}).get(cat,[]):rows.append({'merchant':x.get('merchant') or x.get('name'),'category':cat,'official_homepage':x.get('official_homepage') or x.get('url'),'domain':x.get('domain') or host(x.get('official_homepage') or x.get('url'))})

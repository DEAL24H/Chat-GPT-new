...
def valid_offer(item):
    ...
    if not is_published_verified_offer(item):
        return False
...
def make_article(item):
    ...
    code = clean(item.get("code")); purchase = clean(item.get("final_purchase_url")); promotion_url = clean(item.get("promotion_url"))
    discount = sanitize_visible(item.get("discount")); content = sanitize_visible(item.get("content"))
    ...
    source = clean(item.get("source_url")); source_html = f'<p class="source-note">Verified from the official {esc(merchant)} source.</p>'
    ...
    body = (f'<section class="hero"><p class="eyebrow">{esc(label.upper())}</p><h1>{esc(merchant)} — {esc(title)}</h1><p class="lead">{esc(lead_discount)}{esc(label)} for {esc(merchant)}.</p></section><article><h2>This {esc(label.lower())}</h2><p>{esc(content)}</p>{code_html}<p>{cta}</p>{source_html}{source_link}</article>')
    return canonical, page(f"{merchant} — {title} | DEAL 24H", f"{merchant} {label.lower()}: {title}. Verified official merchant promotion with the correct purchase destination.", canonical, body), label

def main():
    out = ROOT / "seo"
    if out.exists(): shutil.rmtree(out)
    out.mkdir(parents=True, exist_ok=True)
    urls=[]; counts={"code":0,"direct":0}; rejected=0
    for item in load():
        ...
        path.parent.mkdir(parents=True, exist_ok=True); path.write_text(html_text, encoding="utf-8"); urls.append(canonical)
        counts["code" if label=="Promo code" else "direct"] += 1

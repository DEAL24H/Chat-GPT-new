...
def page(title, description, canonical, body):
    copy_js = '''<script>document.querySelectorAll('.copy-code').forEach(function(b){b.addEventListener('click',function(){var c=b.dataset.code||'';navigator.clipboard&&navigator.clipboard.writeText(c).then(function(){b.textContent='Copied';setTimeout(function(){b.textContent='Copy code'},1400)}).catch(function(){var t=document.createElement('textarea');t.value=c;document.body.appendChild(t);t.select();document.execCommand('copy');t.remove();b.textContent='Copied';setTimeout(function(){b.textContent='Copy code'},1400)})})});</script>'''
...
def make_article(item):
    ...
    if not purchase:
        return None
    typ = offer_type(item)
    title = offer_title(item, merchant)
    ...
    code_html = f'''<div class="code"><span><small>CODE</small><strong id="deal-code">{esc(code)}</strong></span><button class="copy-code" type="button" data-code="{esc(code)}">Copy code</button></div>''' if typ == 'code' else ''
    ...
    body = f'''<section class="hero"><p class="eyebrow">{esc(label.upper())}</p><h1>{esc(merchant)} — {esc(title)}</h1><p class="lead">{esc(discount) + ' — ' if discount else ''}{esc(label)} for {esc(merchant)}.</p></section><article><h2>This {esc(label.lower())}</h2><p>{esc(content)}</p>{code_html}<p>{cta}</p>{source_html}{f'<p><a href="{esc(source)}" target="_blank" rel="noopener">View the official source</a></p>' if source else ''}</article>'''
...
def main():
    ...
    (ROOT / 'seo' / 'seo-modes.json').write_text(json.dumps({'generated_at': datetime.now(timezone.utc).isoformat(), 'counts': counts, 'urls': len(urls)}, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')

...
def parse_expiry(text):
    text=clean(text)
    patterns=[r"(?:expires?|expiry|expiration|ends?|valid until|valid through|good through|offer ends?)\s*[:\-]?\s*(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})",r"(?:expires?|expiry|expiration|ends?|valid until|valid through|good through|offer ends?)\s*[:\-]?\s*([A-Za-z]{3,9}\s+\d{1,2}(?:st|nd|rd|th)?(?:,?\s+\d{4})?)"]
...
def main():
    ...
    existing,removed_expired,rejected_non_official=purge_expired(existing); by_key={(d.get("merchant","").lower(),d.get("code","").lower()):d for d in existing if d.get("code")}; changed_sources=0; new_count=0
    for source in SOURCES:
        ...
        try:
            ...
            response=requests.get(source["url"],headers=HEADERS,timeout=25); response.raise_for_status(); html=response.text; digest,changed=source_changed(html,previous)
            state["sources"][source["url"]]={"hash":digest,"last_checked":now(),"last_changed":previous.get("last_changed") or now(),"official_domain":source["domain"]}
            ...
            for deal in deals:
                ...
                if old:
                    old.update({"last_checked":now(),"status":"active","source_url":deal["source_url"],"source_label":deal["source_label"],"source_domain":deal["source_domain"],"official_source":True,"verification_method":"official_merchant_page","code_context":True,"category":deal["category"],"content":deal["content"],"discount":deal["discount"]})
